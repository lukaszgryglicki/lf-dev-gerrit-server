...
do
  ...
  if [ ! -f "mount-${1}-1${z}.json.${STAGE}.secret" ]
  then
    aws --profile lfproduct-${STAGE} efs create-mount-target --file-system-id "${fsid}" --subnet-id "${subnetid}" --security-groups "${sgid}" > "mount-${1}-1${z}.json.${STAGE}.secret"
    res=$?
    if [ ! "${res}" = "0" ]
    then
    ...
    cat "mount-${1}-1${z}.json.${STAGE}.secret"
  fi
done
